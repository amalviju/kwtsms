...
class KWT_SMS_965 {
    ...
    public function send_sms_on_pending_order($order_id) {
        ...
        $message_template = get_option('kwt_sms_pending_message', 'Your order #{order_number} is pending.');
    ...
    public function send_sms_on_processing_order($order_id) {
        $order = wc_get_order($order_id);
        $message_template = get_option('kwt_sms_processing_message', 'Your order #{order_number} is being processed.');
        $message = str_replace('{order_number}', $order->get_order_number(), $message_template);
        $this->send_sms($order, $message);
    }
    ...
    public function send_sms_on_completed_order($order_id) {
        $order = wc_get_order($order_id);
        $message_template = get_option('kwt_sms_completed_message', 'Your order #{order_number} has been completed. Thank you!');
        $message = str_replace('{order_number}', $order->get_order_number(), $message_template);
        $this->send_sms($order, $message);
    }
